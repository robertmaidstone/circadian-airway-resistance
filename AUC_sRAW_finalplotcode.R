...
sum_data_sGAW %>%
  ...
  ylab("AUC sRAW (cm.H<sub>2</sub>O.sec)") + scale_colour_manual(values=col_vec) + scale_fill_manual(values=col_vec) +
...
ggsave(p_pbs + p_hdm,filename="~/Amlan/DCP/p_AUC_sRAW_final.png",width=8,height=5)
...
library(patchwork)
p_pbs + p_hdm
ggsave(p_pbs + p_hdm,filename="~/Amlan/plots_200126/p_AUC_sRAW_final.png",width=8,height=5)
