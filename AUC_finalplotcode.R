...
sum_data %>%
  ...
  geom_ribbon(aes(ymax=Predicted_Response_UCI,ymin=Predicted_Response_LCI,y=NULL,fill=Genotype,colour=NULL),
              data=predict_values_sGAW %>%filter(Treatment=="PBS") %>% mutate(Genotype=factor(Genotype,levels=c("WT","KO"),
...
sum_data %>%
  ...
  geom_point() +
  geom_ribbon(aes(ymax=Predicted_Response_UCI,ymin=Predicted_Response_LCI,y=NULL,fill=Genotype,colour=NULL),
              data=predict_values_sGAW %>%filter(Treatment=="HDM") %>% mutate(Genotype=factor(Genotype,levels=c("WT","KO"),
                                                                                              labels=gen_labels)),size=1,alpha=0.2) +
  geom_line(aes(y=Predicted_Response),
            data=predict_values_sGAW %>%filter(Treatment=="HDM",Genotype=="WT") %>% mutate(Genotype=factor(Genotype,levels=c("WT","KO"),
                                                                                            labels=gen_labels)),size=1) + 
  geom_line(aes(y=Predicted_Response),
            data=predict_values_sGAW %>%filter(Treatment=="HDM",Genotype=="KO") %>% mutate(Genotype=factor(Genotype,levels=c("WT","KO"),
                                                                                                           labels=gen_labels)),size=1,linetype="dashed") + 
...
ggsave(p_pbs + p_hdm,filename="~/Amlan/Flexivent_Jan25/p_AUC_final_v4b.png",width=8,height=5)
...
ggsave(p_pbs + p_hdm,filename="~/Amlan/Flexivent_Jan25/p_AUC_final_v3.png",width=8,height=5)
