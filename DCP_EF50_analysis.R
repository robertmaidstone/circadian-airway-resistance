...
rhy_plot_bar(LF_data,"AUC",y_lim=c(-11,15),"AUC of EF50 (ml.sec<sup>-1</sup>)") -> analysis_out

analysis_out$combined
p_auc <- analysis_out$combined
png("plots_v2/EF50_AUC_bar_v3.png", width = 3000, height = 1500, res = 300)  # adjust size/res as needed
grid.draw(
  p_auc
)
...
rhy_plot_bar(LF_data,"Max",y_lim=c(-.1,.9),"Max EF50 (ml.sec<sup>-1</sup>)") -> analysis_out

analysis_out$combined
p_max <- analysis_out$combined
png("plots_v2/EF50_max_bar.png", width = 3000, height = 1500, res = 300)  # adjust size/res as needed
grid.draw(
  p_max
)
grid.text( "WT", y = unit(0.03, "npc"),x = unit(0.175, "npc"), gp = gpar(fontsize = 12))
