...
aov_emax <- aov(log10(params) ~ ZT * Treatment, data = param_formodel %>% filter(Genotype=="HET",Parameter=="Upper"))
summary(aov_emax)
plot(aov_emax)
...
t_data %>%
  ...
  scale_x_continuous(breaks=c(0,3.12,6.25,12.5,25,50))+
  theme_bw()+
  ylab("Median sRAW (cm.H<sub>2</sub>O.sec)") +
  ylim(c(0.35,0.85))+
...
png("~/Amlan/plots_200126/meth_dose_response_sRAW.png", width = 2400, height = 1500, res = 300)  # adjust size/res as needed
#grid.newpage()
grid.draw(
  patchworkGrob(p1 | p2)
)
