...
LF_data %>% filter(Parameter=="DCP_sGAW") %>% pivot_longer(cols=4:39,names_to = "Animal.ID") %>%
  ...
  dplyr::select(`Animal.ID`,ZT,Min_Value,AUC) %>%
  unique %>% ungroup %>%
  mutate(AUC=Min_Value) %>%
...
sum_data_sGAW %>%
  ...
  ylab("Min sGAW (cm.H<sub>2</sub>O.sec<sup>-1</sup>)") + scale_colour_manual(values=col_vec) + scale_fill_manual(values=col_vec) +
...
ggsave(p_pbs + p_hdm,filename="~/Amlan/DCP/p_Min_sGAW_final.png",width=8,height=5)
...
library(patchwork)
p_pbs + p_hdm
ggsave(p_pbs + p_hdm,filename="~/Amlan/plots_200126/p_Min_sGAW_final.png",width=8,height=5)
