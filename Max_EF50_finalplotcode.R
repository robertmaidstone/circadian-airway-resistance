...
sum_data_sGAW %>%
  ...
  ylab("Max EF50 (ml.sec<sup>-1</sup>)") + scale_colour_manual(values=col_vec) + scale_fill_manual(values=col_vec) +
...
ggsave(p_pbs + p_hdm,filename="~/Amlan/DCP/p_Max_EF50_final.png",width=8,height=5)
...
library(patchwork)
p_pbs + p_hdm
ggsave(p_pbs + p_hdm,filename="~/Amlan/plots_200126/p_max_EF50_final.png",width=8,height=5)
