...
sum_data_sGAW %>%
  ...
  ylab("Max sRAW (cm.H<sub>2</sub>O.sec)") + scale_colour_manual(values=col_vec) + scale_fill_manual(values=col_vec) +
...
sum_data_sGAW %>%
  ...
  ylab("Max Value") + scale_colour_manual(values=col_vec) + scale_fill_manual(values=col_vec) +
  scale_x_continuous(breaks=c(0,6,12,18,24))+
  theme_bw() +
...
ggsave(p_pbs + p_hdm,filename="~/Amlan/DCP/p_Max_sRAW_final.png",width=8,height=5)
...
library(patchwork)
p_pbs + p_hdm
ggsave(p_pbs + p_hdm,filename="~/Amlan/plots_200126/p_max_sRAW_final.png",width=8,height=5)
