...
param_formodel <- dr_fit(LF_data_noninvert) #uncomment these lines to rerun dr curve fits - warning takes some time
save(param_formodel,file = "data/drcmodelparams_flex_LFmaster_ers.RData")
load("data/drcmodelparams_flex_LFmaster_ers.RData")
...
p1<-dr_plot(LF_data,anova_pvals_upper,mw_results,c(0,80),y_lab="Maximum Airway Resistance R<sub>rs</sub>(cm.H<sub>2</sub>O.s.ml<sup>-1</sup>)")
p1

png("plots/flex_meth_dose_response_LFmaster_ers.png", width = 2400, height = 1500, res = 300)  # adjust size/res as needed
grid.draw(
  p1
)
...
rhy_plot(LF_data,"AUC",y_lim=c(-50,120),y_lab="AUC Airway Resistance R<sub>rs</sub>(cm.H<sub>2</sub>O.s.ml<sup>-1</sup>)") -> analysis_out

p_auc <- analysis_out$combined
p_auc
ggsave(p_auc,filename="plots/flex_AUC_LFmaster_ers.png",width=8,height=5)

# Max sinusoidal analysis -----------------------------------------------------

rhy_plot(LF_data,"Max",y_lim=c(-1,4),y_lab="Max Airway Resistance R<sub>rs</sub>(cm.H<sub>2</sub>O.s.ml<sup>-1</sup>)") -> analysis_out

p_max <- analysis_out$combined
p_max
ggsave(p_max,filename="plots/flex_max_LFmaster_ers.png",width=8,height=5)
