...
t_data %>%
  ...
  scale_x_continuous(breaks=c(0,3.12,6.25,12.5,25,50))+
  theme_bw()+
  ylab("Median EF50 (ml.sec<sup>-1</sup>)") +
  ylim(c(-0.1,0.45))+
  ...
  annotate("text", x = 1, y = .45, label = sig_text, hjust = 0, vjust = 1, size = 4)+
...
png("~/Amlan/plots_200126/meth_dose_response_EF50.png", width = 2400, height = 1500, res = 300)  # adjust size/res as needed
#grid.newpage()
grid.draw(
  patchworkGrob(p1 | p2)
)
